...
from object_detection_m_1.core import box_predictor
from object_detection_m_1.utils import static_shape
...
class ConvolutionalBoxPredictor(box_predictor.BoxPredictor):
  ...
  def _predict(self, image_features, num_predictions_per_location_list, audio_features=None):
        ...
        with box_predictor_scope:
          with slim.arg_scope(self._conv_hyperparams_fn()):
            with slim.arg_scope([slim.dropout], is_training=self._is_training):
              ...
              print("before fusion: audio_feature", audio_feature.get_shape())
              # for test
              #inputs = np.random.rand(4,1,1,256).astype(np.float32)
              #audio_feature = [tf.placeholder_with_default(v, v.shape) for v in inputs]
              extended_audio = tf.tile(audio_feature, [1,shape[1],shape[2],1])
              f_net = tf.concat([net, extended_audio], 3)
              print("after fusion: new feature_map", f_net.get_shape())
