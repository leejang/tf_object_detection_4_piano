...
from object_detection_m_1.utils import np_box_list
from object_detection_m_1.utils import np_box_list_ops
from object_detection_m_1.utils import np_box_mask_list
from object_detection_m_1.utils import np_box_mask_list_ops
...
class PerImageEvaluation(object):
  ...
  # to compute the accuracy of note detection for the piano project
  def compute_tp_fp(
      self, detected_boxes, detected_scores, detected_class_labels,
      groundtruth_boxes, groundtruth_class_labels,
      groundtruth_is_difficult_list, groundtruth_is_group_of_list,
    ...
    # temp code to determine the piano project
    # for hanon
    #if (self.num_groundtruth_classes != 22):
    # for six pieces
    if (self.num_groundtruth_classes != 28):
      tp_fp = False
    else:
      print ('compute_tp_fp: True')
      # num_of_notes
      #num_of_note = 7
      num_of_note = 12

      # range of octaves
      #range_of_octave = (17, 22)
      range_of_octave = (22, 28)
      ...
      for i in range(num_of_note):
        ...
        if (len(scores) != 0):
          ...
          tp_fp_label = tp_fp_labels[max_idx]
          note_result_scores.append(max_score)
          note_result_tp_fp_labels.append(tp_fp_label)

      #print note_result_scores
      #print note_result_tp_fp_labels

      # octave detection
      oct_result_scores = []
      oct_result_tp_fp_labels = []
      for i in xrange(range_of_octave[0], range_of_octave[1]):
        groundtruth_is_difficult_list_at_ith_class = (
            groundtruth_is_difficult_list[groundtruth_class_labels == i])
        groundtruth_is_group_of_list_at_ith_class = (
            groundtruth_is_group_of_list[groundtruth_class_labels == i])
        ...
        if (len(scores) != 0):
          ...
          max_score = scores[max_idx]
          tp_fp_label = tp_fp_labels[max_idx]
          oct_result_scores.append(max_score)
          oct_result_tp_fp_labels.append(tp_fp_label)

      #print oct_result_scores
      #print oct_result_tp_fp_labels

      # compute tp fp
      if len(note_result_scores) and len(oct_result_scores):
        note_idx = note_result_scores.index(max(note_result_scores))
        oct_idx = oct_result_scores.index(max(oct_result_scores))
        note_cls = note_result_tp_fp_labels[note_idx]
        oct_cls = oct_result_tp_fp_labels[oct_idx]
        print note_result_scores
        print oct_result_scores
        print note_idx
        print oct_idx
        ...
        # it returns the scores of 2nd max, and max
        # so number [0] means we can pick the index of the 2nd max
        note_2nd_idx = sorted(range(len(note_result_scores)), key=lambda i: note_result_scores[i])[-2:][0]
        oct_2nd_idx = sorted(range(len(oct_result_scores)), key=lambda i: oct_result_scores[i])[-2:][0]

        # pick the 2nd max scores for each task
        note_2nd_score = note_result_scores[note_2nd_idx]
        oct_2nd_score = oct_result_scores[oct_2nd_idx]

        note_2nd_cls = note_result_tp_fp_labels[note_2nd_idx]
        oct_2nd_cls = oct_result_tp_fp_labels[oct_2nd_idx]

        if ((note_cls == True) and (oct_cls == True)):
          # if there are two more detecitons which are higher than decision threshod
          # this is needed especially for two-hands piano playing

          # check the 2nd dections are also true positives
          if (oct_result_tp_fp_labels.count(1.0) > 1):
            if (oct_2nd_cls == True):
              oct_2nd_tp_fp = True
            else:
              oct_2nd_tp_fp = False
          ...
          if (note_result_tp_fp_labels.count(1.0) > 1):
            if (note_2nd_cls == True):
              note_2nd_tp_fp = True
            else:
              note_2nd_tp_fp = False
          else:
            note_2nd_tp_fp = True

          if ((oct_2nd_tp_fp == True) and (note_2nd_tp_fp == True)):
            tp_fp = True
          else:
            tp_fp = False
        elif ((oct_result_tp_fp_labels.count(1.0) == 0) and (note_result_tp_fp_labels.count(1.0) == 0)):
          tp_fp = True
        else:
          tp_fp = False
      # No detections
      # In this case, there is no detection result
      # This is not happened in usual case,
      # since detector tried to check all test images
      # So, need to check input images when it happens
      else:
        if ((oct_result_tp_fp_labels.count(1.0) == 0) and (note_result_tp_fp_labels.count(1.0) == 0)):
          tp_fp = True
        else:
          tp_fp = False
