...
from object_detection_m_1.core import standard_fields
from object_detection_m_1.utils import label_map_util
from object_detection_m_1.utils import metrics
from object_detection_m_1.utils import per_image_evaluation
...
class ObjectDetectionEvaluator(DetectionEvaluator):
  ...
  def _build_metric_names(self):
    ...
    # note detection accuracy for the piano project
    self._metric_names.append('NoteDetectionAccuracy')
  ...
  def evaluate(self):
    ...
    (per_class_ap, mean_ap, per_class_precision, per_class_recall,
     per_class_corloc, mean_corloc, note_cls_acc) = (
    ...
    # Add note_cls_accuracy
    pascal_metrics['NoteDetectionAccuracy'] = note_cls_acc
...
class ObjectDetectionEvaluation(object):
  ...
  def __init__(self,
    ...
    # number of condition negative images
    # which do not have any gt classes (background)
    # true negatives
    self.num_tn_images = 0
    # false positive
    self.num_fp_images = 0

    # number of condition positive images
    # true positives
    self.num_tp_images = 0
    # false negatives
    self.num_fn_images = 0
  ...
  def add_single_detected_image_info(self, image_key, detected_boxes,
    ...
    # add compute_tp_fn for note detection
    tp_fp = (
        self.per_image_eval.compute_tp_fp(
            detected_boxes=detected_boxes,
            detected_scores=detected_scores,
            detected_class_labels=detected_class_labels,
            groundtruth_boxes=groundtruth_boxes,
    ...
    if (tp_fp == True):
      self.num_tp_images += 1
      #print ("tp_images")
    else:
      self.num_fp_images += 1
      #print ("fp_images")
  ...
  def evaluate(self):
    ...
    # check all gt images
    for gt_img_key in self.groundtruth_boxes:
      # no detection for this gt_image
      if gt_img_key not in self.detection_keys:
        # no grount truth labels (background) - true negative (tn)
        if (len(self.groundtruth_class_labels[gt_img_key]) == 0):
          self.num_tn_images += 1
          #print gt_img_key
        else:
          #print gt_img_key
          #print self.groundtruth_class_labels[gt_img_key]
          gt_notes = []
          for gt_label in self.groundtruth_class_labels[gt_img_key]:
            # check if there exist the gt labels of note classification
            if gt_label < 7:
              gt_notes.append(gt_label)

          if (len(gt_notes) != 0):
            self.num_fn_images += 1
          # no detection results and no gt labels for note classfication
          else:
            self.num_tn_images += 1

    # compute accuracy at here
    note_cls_accuracy =  (float(self.num_tp_images + self.num_tn_images) /
      float(self.num_tp_images + self.num_tn_images+ self.num_fp_images + self.num_fn_images))
    print('[piano] TP: %d', self.num_tp_images)
    print('[piano] TN: %d', self.num_tn_images)
    print('[piano] FP: %d', self.num_fp_images)
    print('[piano] FN: %d', self.num_fn_images)
    logging.info('[piano] note classification accuracy: %f', note_cls_accuracy)
    print('[piano] note classification accuracy: %f', note_cls_accuracy)

    return ObjectDetectionEvalMetrics(
        self.average_precision_per_class, mean_ap, self.precisions_per_class,
        self.recalls_per_class, self.corloc_per_class, mean_corloc, note_cls_accuracy)
