...
from object_detection_m1.builders import dataset_builder
from object_detection_m1.builders import image_resizer_builder
from object_detection_m1.builders import model_builder
from object_detection_m1.builders import preprocessor_builder
from object_detection_m1.core import preprocessor
from object_detection_m1.core import standard_fields as fields
from object_detection_m1.data_decoders import tf_example_decoder
from object_detection_m1.protos import eval_pb2
from object_detection_m1.protos import input_reader_pb2
from object_detection_m1.protos import model_pb2
from object_detection_m1.protos import train_pb2
from object_detection_m1.utils import config_util
from object_detection_m1.utils import ops as util_ops
from object_detection_m1.utils import shape_utils
...
def transform_input_data(tensor_dict,
  ...
  # audio input (no resize)
  if fields.InputDataFields.audio in tensor_dict:
    audio = tensor_dict[fields.InputDataFields.audio]
    if use_bfloat16:
      audio = tf.cast(audio, tf.bfloat16)
    float_audio = tf.to_float(audio)
    tensor_dict[fields.InputDataFields.audio] = float_audio

  # flow input
  flow = tensor_dict[fields.InputDataFields.flow]
  preprocessed_resized_flow, true_flow_shape = model_preprocess_fn(
      tf.expand_dims(tf.to_float(flow), axis=0))
  if use_bfloat16:
    preprocessed_resized_flow = tf.cast(
        preprocessed_resized_flow, tf.bfloat16)
  tensor_dict[fields.InputDataFields.flow] = tf.squeeze(
      preprocessed_resized_flow, axis=0)

  # image 1 and flow 1 (previous frame)
  image_1 = tensor_dict[fields.InputDataFields.image_1]
  preprocessed_resized_image_1, true_image_shape = model_preprocess_fn(
      tf.expand_dims(tf.to_float(image_1), axis=0))
  if use_bfloat16:
    preprocessed_resized_image_1 = tf.cast(
        preprocessed_resized_image_1, tf.bfloat16)
  tensor_dict[fields.InputDataFields.image_1] = tf.squeeze(
      preprocessed_resized_image_1, axis=0)

  flow_1 = tensor_dict[fields.InputDataFields.flow_1]
  preprocessed_resized_flow_1, true_flow_shape = model_preprocess_fn(
      tf.expand_dims(tf.to_float(flow_1), axis=0))
  if use_bfloat16:
    preprocessed_resized_flow_1 = tf.cast(
        preprocessed_resized_flow_1, tf.bfloat16)
  tensor_dict[fields.InputDataFields.flow_1] = tf.squeeze(
      preprocessed_resized_flow_1, axis=0)

  # image 2 and flow 2 (2 frames before)
  image_2 = tensor_dict[fields.InputDataFields.image_2]
  preprocessed_resized_image_2, true_image_shape = model_preprocess_fn(
      tf.expand_dims(tf.to_float(image_2), axis=0))
  if use_bfloat16:
    preprocessed_resized_image_2 = tf.cast(
        preprocessed_resized_image_2, tf.bfloat16)
  tensor_dict[fields.InputDataFields.image_2] = tf.squeeze(
      preprocessed_resized_image_2, axis=0)

  flow_2 = tensor_dict[fields.InputDataFields.flow_2]
  preprocessed_resized_flow_2, true_flow_shape = model_preprocess_fn(
      tf.expand_dims(tf.to_float(flow_2), axis=0))
  if use_bfloat16:
    preprocessed_resized_flow_2 = tf.cast(
        preprocessed_resized_flow_2, tf.bfloat16)
  tensor_dict[fields.InputDataFields.flow_2] = tf.squeeze(
      preprocessed_resized_flow_2, axis=0)
...
def pad_input_data_to_static_shapes(tensor_dict, max_num_boxes, num_classes,
  ...
  padding_shapes = {
      ...
      fields.InputDataFields.audio: [
          height, width, num_image_channels + num_additional_channels
      ],
      fields.InputDataFields.flow: [
          height, width, num_image_channels + num_additional_channels
      ],
      fields.InputDataFields.image_1: [
          height, width, num_image_channels + num_additional_channels
      ],
      fields.InputDataFields.flow_1: [
          height, width, num_image_channels + num_additional_channels
      ],
      fields.InputDataFields.image_2: [
          height, width, num_image_channels + num_additional_channels
      ],
      fields.InputDataFields.flow_2: [
          height, width, num_image_channels + num_additional_channels
      ],
  ...
  """
  if fields.InputDataFields.audio in tensor_dict:
    padding_shapes[fields.InputDataFields.audio] = [
        20, 9, 3
    ]
  """
  if fields.InputDataFields.audio in tensor_dict:
...
def _get_features_dict(input_dict):
  ...
  # audio
  if fields.InputDataFields.audio in input_dict:
    features[fields.InputDataFields.audio] = input_dict[
        fields.InputDataFields.audio]
  # flow
  if fields.InputDataFields.flow in input_dict:
    features[fields.InputDataFields.flow] = input_dict[
        fields.InputDataFields.flow]

  # image 1
  if fields.InputDataFields.image_1 in input_dict:
    features[fields.InputDataFields.image_1] = input_dict[
        fields.InputDataFields.image_1]
  # flow 1
  if fields.InputDataFields.flow_1 in input_dict:
    features[fields.InputDataFields.flow_1] = input_dict[
        fields.InputDataFields.flow_1]

  # image 2
  if fields.InputDataFields.image_2 in input_dict:
    features[fields.InputDataFields.image_2] = input_dict[
        fields.InputDataFields.image_2]
  # flow 2
  if fields.InputDataFields.flow_2 in input_dict:
    features[fields.InputDataFields.flow_2] = input_dict[
        fields.InputDataFields.flow_2]
...
def create_predict_input_fn(model_config, predict_input_config):
  ...
  def _predict_input_fn(params=None):
    ...
    # flow
    if fields.InputDataFields.audio in input_dict:
      flows = tf.to_float(input_dict[fields.InputDataFields.flow])
      flows = tf.expand_dims(flows, axis=0)

    # image 1 and flow 1
    images_1 = tf.to_float(input_dict[fields.InputDataFields.image_1])
    images_1 = tf.expand_dims(images_1, axis=0)
    flows_1 = tf.to_float(input_dict[fields.InputDataFields.flow_1])
    flows_1 = tf.expand_dims(flows_1, axis=0)

    # image 2 and flow 2
    images_2 = tf.to_float(input_dict[fields.InputDataFields.image_2])
    images_2 = tf.expand_dims(images_2, axis=0)
    flows_2 = tf.to_float(input_dict[fields.InputDataFields.flow_2])
    flows_2 = tf.expand_dims(flows_2, axis=0)

    # audios
    if fields.InputDataFields.audio in input_dict:
      audios = tf.to_float(input_dict[fields.InputDataFields.audio])
      audios = tf.expand_dims(audios, axis=0)
      return tf.estimator.export.ServingInputReceiver(
          features={
              fields.InputDataFields.image: images,
              fields.InputDataFields.audio: audios,
              fields.InputDataFields.flow: flows,
              fields.InputDataFields.image_1: images_1,
              fields.InputDataFields.flow_1: flows_1,
              fields.InputDataFields.image_2: images_2,
              fields.InputDataFields.flow_2: flows_2,
