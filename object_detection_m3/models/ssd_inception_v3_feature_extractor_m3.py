...
from object_detection_m3.models import feature_map_generators
from object_detection_m3.utils import ops
from object_detection_m3.utils import shape_utils
from nets import inception_v3_m3

slim = tf.contrib.slim


class SSDInceptionV3FeatureExtractorM3(ssd_meta_arch.SSDFeatureExtractor):
  """SSD Feature Extractor using InceptionV3 features with MultiTasks"""
  ...
  def __init__(self,
    ...
    """
    super(SSDInceptionV3FeatureExtractorM3, self).__init__(
  ...
  def extract_audio_features(self, inputs):
    ...
    with tf.variable_scope('audioNet', [inputs]):
      ...
      # for hanon
      #net = slim.conv2d(net, 2048, [12, 5], padding="VALID", scope='fc3')
      # for six-pieces
      net = slim.conv2d(net, 2048, [12, 6], padding="VALID", scope='fc3')
      end_points['fc3'] = net

      net = slim.conv2d(net, 512, [1, 1], scope='fc4')
      end_points['fc4'] = net
  ...
  def extract_features(self, preprocessed_inputs, audio_inputs=None, preprocessed_flows=None,
                       preprocessed_inputs_1=None, preprocessed_flows_1=None,
                       preprocessed_inputs_2=None, preprocessed_flows_2=None):
    ...
    with slim.arg_scope(self._conv_hyperparams_fn()):
      with tf.variable_scope('InceptionV3', reuse=self._reuse_weights) as scope:
       ...
       """
       # M3
       _, image_features = inception_v3_m3.inception_v3_base(
            ops.pad_to_multiple(preprocessed_inputs, self._pad_to_multiple),
            ops.pad_to_multiple(preprocessed_flows, self._pad_to_multiple),
            ops.pad_to_multiple(preprocessed_inputs_1, self._pad_to_multiple),
            ops.pad_to_multiple(preprocessed_flows_1, self._pad_to_multiple),
            ops.pad_to_multiple(preprocessed_inputs_2, self._pad_to_multiple),
            ops.pad_to_multiple(preprocessed_flows_2, self._pad_to_multiple),
            final_endpoint='Mixed_7c',
            min_depth=self._min_depth,
            depth_multiplier=self._depth_multiplier,
            scope=scope)
