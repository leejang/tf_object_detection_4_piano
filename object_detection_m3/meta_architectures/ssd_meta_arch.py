...
class SSDMetaArch(model.DetectionModel):
  ...
  # M1
  #def predict(self, preprocessed_inputs, true_image_shapes, audio_inputs=None):
  # M2
  #def predict(self, preprocessed_inputs, true_image_shapes, audio_inputs=None, flow_inputs=None):
  # M3
  def predict(self, preprocessed_inputs, true_image_shapes, audio_inputs=None, flow_inputs=None,
              preprocessed_inputs_1=None, flow_inputs_1=None,
              preprocessed_inputs_2=None, flow_inputs_2=None):
    ...
    if self._feature_extractor.is_keras_model:
      ...
      with slim.arg_scope([slim.batch_norm],
        ...
        with tf.variable_scope(None, self._extract_features_scope,
                               [preprocessed_inputs]):
          # M3
          if ((preprocessed_inputs_1 != None) and (flow_inputs_1 != None)):
            feature_maps, audio_features = self._feature_extractor.extract_features(
                preprocessed_inputs, audio_inputs, flow_inputs,
                preprocessed_inputs_1, flow_inputs_1,
                preprocessed_inputs_2, flow_inputs_2)
          elif ((audio_inputs != None) and (flow_inputs != None)):
            feature_maps, audio_features = self._feature_extractor.extract_features(
                preprocessed_inputs, audio_inputs, flow_inputs)
          elif (audio_inputs != None):
            feature_maps, audio_features = self._feature_extractor.extract_features(
                preprocessed_inputs, audio_inputs)
          else:
    ...
    if self._box_predictor.is_keras_model:
      ...
      with slim.arg_scope([slim.batch_norm],
        ...
        if (audio_inputs != None):
          #print ("audio_inputs:", audio_inputs.get_shape())
          predictor_results_dict = self._box_predictor.predict(
              feature_maps, self._anchor_generator.num_anchors_per_location(), audio_features=audio_features)
        else:
          predictor_results_dict = self._box_predictor.predict(
